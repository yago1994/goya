...
export default function App() {
  ...
  function connectTargetAt(e: React.PointerEvent, excludeId: string): string | null {
    ...
    // A shape is usually the backdrop other things sit on, so it only wins the
    // drop when nothing else is in reach — otherwise dragging towards a note
    // that sits on a big rectangle would attach to the rectangle instead.
  ...
  function handleRootPointerMove(e: React.PointerEvent) {
    ...
    if (g.mode === 'pan') {
      ...
      const target = docRef.current.elements.find((x) => x.id === g.id)
      if (e.shiftKey) {
      ...
      const thin = target?.type === 'draw'
      nw = Math.max(thin ? 8 : 40, nw)
      nh = Math.max(thin ? 8 : 28, nh)
  ...
  async function runMiroExport() {
    try {
      const res = await exportBoardVsdx(activeBoard.name, docRef.current)
      const notes: string[] = []
      if (res.missingImages.length > 0) {
        const c = res.missingImages.length
        notes.push(`${c} image${c > 1 ? 's' : ''} could not be embedded (blocked by the host).`)
      }
      if (res.skipped.length > 0) notes.push(`${res.skipped.length} element(s) had no Visio equivalent.`)
      window.alert(
        'Saved a .vsdx file.\n\n' +
          'In Miro: Shapes > More shapes > Import diagram\n' +
